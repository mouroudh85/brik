...
if not API_KEY:
    st.error("❌ Clé API manquante. Ajoutez GOOGLE_API_KEY dans le fichier .env")
    st.stop()
...
def show_artisan_dashboard():
    ...
    # Tabs pour l'interface artisan
    tab1, tab2 = st.tabs(["📋 Demandes de travaux", "💬 Assistant IA"])
...
def show_chat_assistant():
    ...
    _show_chat_interface("chat_messages")

def show_chat_assistant_artisan():
    st.subheader("💬 Assistant IA - Conseils pour vos devis")
    st.info("🤖 Je peux vous aider à estimer les coûts, calculer les matériaux, conseiller sur les délais, etc.")
    _show_chat_interface("chat_messages_artisan")

def _show_chat_interface(session_key):
    ...
    # Zone de saisie
    prompt_text = "Posez votre question (ex: Quel est le prix moyen pour repeindre une pièce ?)" if session_key == "chat_messages" else "Posez votre question (ex: Comment calculer les m² de carrelage nécessaires ?)"
...
Sois concis (max 200 mots) et pratique."""
                        ...
                        context = """Tu es un assistant expert pour artisans du bâtiment.
Tu aides les artisans à calculer les matériaux, estimer les coûts, évaluer les délais.
Donne des conseils professionnels précis avec des calculs détaillés.
Sois concis (max 200 mots) et pratique."""
    ...
    if st.button("🔄 Nouvelle conversation", key=f"reset_{session_key}"):
        st.session_state[session_key] = []
        st.rerun()
